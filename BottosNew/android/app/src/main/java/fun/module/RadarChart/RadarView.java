...
public class RadarView extends View {
    ...
    private int count = 6;//雷达网圈数valuePointRadius
    ...
    private int mainColor = 0xFFAB99C1;//雷达区颜色
    private int valueColor = 0xFF947CAF;//数据区颜色
    private int textColor = 0xFFAB99C1;//文本颜色

    private float mainLineWidth = 1f;//雷达网线宽度dp
    private float valueLineWidth = 1f;//数据区边宽度dp
    private float valuePointRadius = 1f;//数据区圆点半径dp
    ...
    private void drawText(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            float y = (float) ((radius + fontHeight * 1) * Math.cos(angle / 2 + angle * i));
