...
import GroupLevel from '../../Community/Components/GroupLevel' // 用户等级
import RedPacketPostItem from '../../Community/RedPacketSend/RedPacketPostItem' // 红包
import DoubleSourceImage from '../../Community/DoubleSourceImage' // 图片
import ImageShow from '../../Community/ImageShow'
import PostComment from '../../Community/Components/PostComment' // 评论
import URLShareCell from '../../Community/Components/URLShareCell' // 分享
import PraiseAndCommentItem from './PraiseAndCommentItem' // 点赞&&评论

class PostListCell extends PureComponent {
  ...
  _renderTopAndBoutique(is_top, is_boutique) {
    let imagURL = null
    if (is_top) {
      imagURL = require('../../BTImage/Homepage/homepage_icon_top.png')
    } else if (is_boutique) {
      imagURL = require('../../BTImage/Homepage/homepage_icon_boutique.png')
    }
    return (
      <Image
        resizeMode="contain"
        style={{ width: 37, height: 37, position: 'absolute', right: 0 }}
        source={imagURL}
      />
    )
  }
  render() {
    ...
    return (
      <View style={styles.container}>
        {/* 置顶 加精 */}
        {is_top || is_boutique ? (
          this._renderTopAndBoutique(is_top, is_boutique)
        ) : (
          <View />
        )}
...
export default connect(mapStateToProps)(PostListCell)
