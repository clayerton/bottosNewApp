...
import { Text, View, StyleSheet, Keyboard, FlatList, TouchableOpacity, } from 'react-native';
...
class PostComment extends PureComponent {
    ...
    render() {
        ...
        return (React.createElement(View, { style: styles.reply },
            React.createElement(FlatList, { data: isShowReplyList ? replys : replyTemp, extraData: isShowReplyList ? replys : replyTemp, renderItem: ({ item }) => {
                    return (React.createElement(AllPostCommentItem, { post_id: post_id, scrollToIndex: scrollToIndex, item: item, routeName: routeName }));
                }, keyExtractor: item => item.reply_id.toString() }),
            haveMoreReply && (React.createElement(TouchableOpacity, { style: {
                    alignItems: 'center',
                    justifyContent: 'center',
                    height: 20,
                    marginTop: 8
                }, onPress: () => this.onPressShowReplyList() },
                React.createElement(Text, { style: { fontSize: 14, color: '#1677CB' } }, isShowReplyList ? '收起' : '显示更多')))));
    }
}
const mapDispatchToProps = dispatch => {
    return {
        addComment(replyDetail) {
