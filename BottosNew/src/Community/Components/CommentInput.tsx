...
interface Props extends OwnProps {
  targetPostId: number // 回复的目标帖子 ID
  follow_id: number // 回复的目标的 ID
  targetName: string
 ...
 *
 */
class CommentInput extends PureComponent<Props, State> {
  ...
  _keyboardDidShow(e) {
    ...
    if (onkeyboardHeightChange) onkeyboardHeightChange(keyboardHeight)
  }

  _keyboardDidHide() {
    devlog('Keyboard Hidden')
    const onkeyboardDidHide = this.props.onkeyboardDidHide
    if (onkeyboardDidHide) onkeyboardDidHide()
  ...
  render() {
    ...
    const placeholderText =
      member_name && I18n.t('community.reply') + member_name + '：'

    return (
      <View style={styles.wrapper}>
        <TextInput
          style={[styles.inputView, styles.textBorder]}
          ...
          returnKeyType="send"
          onSubmitEditing={this.sendCommit}
          autoFocus={true}
        ...
        />
        <TouchableOpacity activeOpacity={0.5} onPress={this.sendCommit}>
          <Text style={[styles.enter, styles.textBorder]}>
            {I18n.t('community.send')}
          </Text>
        </TouchableOpacity>
      </View>
    )
...
const mapStateToProps = state => {
  ...
  var targetName = '0000'
  if (follow_id != -1) {
    const post = HomePageContentList.data.find(
      ele => ele.post_id == targetPostId
    )
