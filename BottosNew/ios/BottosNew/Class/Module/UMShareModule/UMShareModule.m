...
//add by zzl 判断是否安装分享平台
RCT_EXPORT_METHOD(isInstallShareAPP:(NSInteger)platform completion:(RCTResponseSenderBlock)completion)
{
  UMSocialPlatformType plf = [self platformType:platform];
  if (plf == UMSocialPlatformType_UnKnown) {
    if (completion) {
  ...
  BOOL isInstall = [[UMSocialManager defaultManager] isInstall:plf];
  if (isInstall) {
    completion([NSArray arrayWithObjects:@"1", nil]);
  }else{
    completion([NSArray arrayWithObjects:@"0", nil]);
  }
}
